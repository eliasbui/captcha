...
class CRNN(nn.Module):
    def __init__(self, num_chars, rnn_hidden_size=256, dropout=0.1, use_attention=True):
        ...
        self.cnn_p1 = nn.Sequential(
            *list(base.features[:6])
            )  # First few layers
        
        # Enhanced conv layer with residual connections
    ...
    def forward(self, batch):
        ...
        batch = self.adaptive_pool(batch)  # (B, C=256, H=1, W=time)
        
        # Reshape for RNN: (B, W, C)
        batch = batch.squeeze(2).permute(0, 2, 1)  # (B, time, 256)

        # # Feature projection
        # batch = self.feature_proj(batch)
        
        # RNN layers with residual connections
        rnn1_out, _ = self.rnn1(batch)
